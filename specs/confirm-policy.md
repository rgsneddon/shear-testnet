...
Surfaces that share one table: wallet Continuum / Shearview, pool pending / confirmed, explorer.shear.digital, Join “claim paid”, node RPC `getpolicy`. Pool dashboard is not the source of `d_max` / `h_ratio` — those are measured from headers and `getchaintips`.
...
| Exchange / Join mark-paid | 200 | ~5 h | Join vault debit |
...
- `d_max >= 10` **or** `side_lead > 0` for more than 2 block times (180 s) → **freeze credits** (pool confirmed, Join mark-paid, wallet spendable stays pending even past 6)
...
```
{
  consensus_min: 6,
  merchant_default: 12,
  bands: { ui_seen, consensus_spendable, peer_small_flow, pool_merchant, otc_large, join_mark_paid },
  frozen, d_max, h_ratio, side_lead, reorg_risk,
  operational: { pool_merchant, join_mark_paid, ... }
}
```
...
Join mark-paid waits for operational `join_mark_paid` (default 200) **and** `frozen == false`. Snapshot line paid at most once (`join.md`).
