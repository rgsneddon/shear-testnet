...
The continuity root commits these samples. After 1000 confirmations the **sample list** may be pruned; **coinbase outputs and every user transfer stay sealed forever**. Explorer reports those sealed rows for eternity. It never needs the pruned per-hash JSON.
...
- If fewer than 99 days remain, new deposits still lock, but they sit idle: no interest. They can still vote.
