...
ShearK implements ShearHash-v3. Banner `ShearK-Miner 2.5 (ShearHash-v3 light)`. `--print-config` `personalisation=ShearHash-v3`, `rxMode=light`. Default `--backend jit-full` (2 GiB dataset). Pool/node verify light. Share floor is dest-bound (`SHARE_BIND=rx+noteCommit`).
