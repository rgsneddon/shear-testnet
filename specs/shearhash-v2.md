...
**ShearK-Miner** `1.6` (two-part). Banner: `ShearK-Miner 1.6 (ShearHash-v2 light)`. Official downloads: `rgsneddon/ShearK` tag **1.6**. Submit carries the ShearHash-v2 digest. One share in flight; `busy` is retried. A timestamp restamp does not rebuild RandomX K.
