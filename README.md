...
Windows ops start: [`HANDOFF_OPS.md`](HANDOFF_OPS.md). MacBook cuts Apple-only artifacts (Continuum 0.49 `.dmg`/CLI, iOS if cutting, ShearK 2.5 macOS): [`CONTINUUM-0.46-MAC-HANDOFF.md`](CONTINUUM-0.46-MAC-HANDOFF.md).
...
- Wallet pin: **0.49** (store `0.49.0+71`, GUI + CLI). Releases: https://github.com/rgsneddon/shear-testnet/releases/tag/0.49 — notes in [`wallet/RELEASES.md`](wallet/RELEASES.md) and [`CONTINUUM-0.49-HANDOFF.md`](CONTINUUM-0.49-HANDOFF.md)
...
Wallet **0.49** syncs a local node at `127.0.0.1:18332` (headers + compact blocks). It does not use the old height sampler. Public pool HTTP submit is an advanced toggle. Reserve Send shows your IP to the node. Use a VPN, then confirm **I already use a VPN / I accept exposing my IP** before Send. There is no Shear privacy hop.
