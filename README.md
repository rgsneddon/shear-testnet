...
Windows ops start: [`HANDOFF_OPS.md`](HANDOFF_OPS.md). MacBook cuts Apple-only artifacts (Continuum 0.43 `.dmg`/CLI, iOS if cutting, ShearK 2.5 macOS): [`CONTINUUM-0.43-MAC-HANDOFF.md`](CONTINUUM-0.43-MAC-HANDOFF.md).
...
- Wallet pin: **0.43** (GUI + CLI). Releases: https://github.com/rgsneddon/shear-testnet/releases/tag/0.43
...
Wallet **0.43** syncs a local node at `127.0.0.1:18332` (headers + compact blocks). It does not use the old height sampler. Public pool HTTP submit is an advanced toggle. Optional Reserve Privacy hop (0.05 SHE) hides device IP; Send without hop confirms you already use a VPN.
