...
**Working branch:** `main`  
**This file is the Windows start for Shear.** Miner binaries stay in **`rgsneddon/ShearK`**. Pins are **0.49** (wallet) and ShearK **2.5**. Pin **0.49**. Do not recut older tags. The MacBook cuts **macOS only** (`CONTINUUM-0.46-MAC-HANDOFF.md`). This Windows box packs APK, Windows zip, Linux/Arch, and site pins. Continuum 0.49 download and acceptance: `CONTINUUM-0.49-HANDOFF.md`.
...
1. Pack **wallet 0.49** Windows/Android on this box onto tag `0.49`. Linux/Arch on `77.42.91.84`. Darwin cannot `flutter build windows`. Handoff: `CONTINUUM-0.49-HANDOFF.md`.
2. **Android 0.49 APK is packed on this Windows box** (Flutter + Android SDK 36 + Eclipse Temurin JDK 17). See §6a. Do not wait for the Mac for Android.
3. **Apple-only artifacts stay a MacBook cut** (`CONTINUUM-0.46-MAC-HANDOFF.md`): Continuum 0.49 `.dmg` + CLI, iOS if cutting, ShearK-Miner 2.5 macOS zip. Do not recut 0.48.
...
- Wallet Flutter **3.47.x** (or 3.44.6). Pin **0.49**. `--build-name=0.49.0` / pubspec `0.49.0+71`.
...
copy /Y build\app\outputs\flutter-apk\app-release.apk dist\shear-wallet-0.49-android.apk
copy /Y build\app\outputs\flutter-apk\app-release.apk ..\dist\shear-wallet-0.49-android.apk
gh release upload 0.49 dist\shear-wallet-0.49-android.apk --repo rgsneddon/shear-testnet --clobber
```

Fat APK only (`flutter build apk`, not `--split-per-abi`). `applicationId` `com.shear.shear_wallet`. Uninstall any old debug-signed build before sideload. Tag `0.49` ships `shear-wallet-0.49-android.apk`.
...
- Recut an older wallet or ShearK tag. Pins are **0.49** and **2.5**. Do not recut 0.48.
