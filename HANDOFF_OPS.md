...
**Working branch:** `main`  
**This file is the Windows start for Shear.** Miner binaries stay in **`rgsneddon/ShearK`**. Pins are **0.46** (wallet) and ShearK **2.5**. Pin **0.46**. Do not recut older tags. The MacBook cuts **macOS only** (`CONTINUUM-0.46-MAC-HANDOFF.md`). This Windows box packs APK, Windows zip, Linux/Arch, and site pins.
...
| Wallet | **0.46** (`wallet/lib/main.dart` `kWalletVersion`) |
...
Wallet zip on **this** repo: https://github.com/rgsneddon/shear-testnet/releases/tag/0.46  
...
1. Pack **wallet 0.46** Windows/Android on this box onto tag `0.46`. Linux/Arch on `77.42.91.84`. Darwin cannot `flutter build windows`.
2. **Android 0.46 APK is packed on this Windows box** (Flutter + Android SDK 36 + Eclipse Temurin JDK 17). See §6a. Do not wait for the Mac for Android.
3. **Apple-only artifacts stay a MacBook cut** (`CONTINUUM-0.46-MAC-HANDOFF.md`): Continuum 0.46 `.dmg` + CLI, iOS if cutting, ShearK-Miner 2.5 macOS zip. Do not recut 0.45.
...
- Wallet Flutter **3.47.x** (or 3.44.6). Pin **0.46**. `--build-name=0.46.0` / pubspec `0.46.0+63`.
...
flutter pub get
flutter build apk --release --build-name=0.46.0 --build-number=63
copy /Y build\app\outputs\flutter-apk\app-release.apk dist\shear-wallet-0.46-android.apk
copy /Y build\app\outputs\flutter-apk\app-release.apk ..\dist\shear-wallet-0.46-android.apk
gh release upload 0.46 dist\shear-wallet-0.46-android.apk --repo rgsneddon/shear-testnet --clobber
```

Fat APK only (`flutter build apk`, not `--split-per-abi`). `applicationId` `com.shear.shear_wallet`. Uninstall any old debug-signed build before sideload. Tag `0.46` has `shear-wallet-0.46-android.apk`.
...
- Recut an older wallet or ShearK tag. Pins are **0.46** and **2.5**. Do not recut 0.45.
...
- Soak checklist (watch, not code): Continuum 0.46 on live worker Copy dest — mine → ≥6 conf → ShearView row → Resistance; fail release if empty after sealed hash notes. ASERT settle claim only after ≥288 blocks. Thin |J| until ≥10k.
...
Magic stays **`shear-testnet-v4`**. Keep Continuum pin 0.46 / ShearK 2.5. Wipe every datadir together, restore the pool ident from the Desktop vault onto the pool box, start the three P2P nodes, then the pool.
