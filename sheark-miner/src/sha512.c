...
static void store64_be(unsigned char *p, uint64_t v) {
  for (int i = 7; i >= 0; i--) {
...
static const uint64_t K[80] = {
  ...
  0x391c0cb3c5c95a63ULL, 0x4ed8aa4ae256485bULL, 0x5b9cca4f7763e373ULL, 0x682e6ff3d6b2b8a3ULL,
...
static void compress(uint64_t s[8], const unsigned char block[128]) {
  uint64_t w[80];
...
void shear_sha512(const unsigned char *msg, size_t len, unsigned char out[64]) {
  ...
  store64_be(block + 120, (uint64_t)len * 8ULL);
  compress(s, block);
  for (int i = 0; i < 8; i++) store64_be(out + i * 8, s[i]);
}
