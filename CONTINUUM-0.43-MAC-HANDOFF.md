# Continuum 0.43 + ShearK 2.5 — MacBook only (Apple parts)
...
**Wallet tag:** `0.43` — https://github.com/rgsneddon/shear-testnet/releases/tag/0.43  
**Miner repo:** https://github.com/rgsneddon/ShearK  
**Miner tag:** `2.5` — https://github.com/rgsneddon/ShearK/releases/tag/2.5  
**This file:** https://github.com/rgsneddon/shear-testnet/blob/main/CONTINUUM-0.43-MAC-HANDOFF.md
...
## A) Continuum 0.43 macOS GUI (notarized `.dmg`)
...
# writes wallet/dist/shear-wallet-0.43-macos.dmg
gh release upload 0.43 dist/shear-wallet-0.43-macos.dmg --repo rgsneddon/shear-testnet --clobber
```
...
URL: `https://github.com/rgsneddon/shear-testnet/releases/download/0.43/shear-wallet-0.43-macos.dmg`

Then on `main`, replace any remaining site span `macOS — coming soon` with that href.

## B) Continuum 0.43 macOS CLI

Windows cannot emit a Darwin Mach-O.

```
cd ~/shear-testnet/wallet
dart compile exe bin/shear.dart -o dist/shear-0.43-macos
chmod +x dist/shear-0.43-macos
./dist/shear-0.43-macos --version    # 0.43
gh release upload 0.43 dist/shear-0.43-macos --repo rgsneddon/shear-testnet --clobber
```
...
The Windows box cannot produce an IPA. If this 0.43 cut includes iOS:

```
cd ~/shear-testnet/wallet
flutter build ipa --release --build-name=0.43.0 --build-number=59
...
Official miner is a **separate** ShearK release — never inside the wallet. Windows already ships `ShearK-Miner-2.5-windows.zip`. Linux is packed on `77.42.91.84`. **macOS miner is MacBook-only.**
...
Wallet public pin **0.43** (`kWalletVersion` / `kCliVersion`), store `0.43.0+60`. Miner pin **ShearK 2.5**.
