...
**Versioning:** two integers only (`*.*`, never `*.*.*`). Start at **0.1**. After `0.9` comes `0.10`, `0.11`, `0.12`, … — do not roll to `1.0`. Wallet / node / pool **0.1**. Miner **0.5**. Never `0.1.0`. Never bump to `1.*` unless the operator says so. Every GitHub release title is **`[testnet] …`**.
