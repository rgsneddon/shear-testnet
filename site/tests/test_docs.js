...
describe('shear.digital/docs', () => {
  it('is a file-tree browser with wallet, mining, vortex, vort1, reserve', () => {
    ...
    assert.match(content, /Wallet pin<\/th><td>0\.43/);
    assert.match(content, /Current pin is <strong>0\.43<\/strong>/);
    assert.match(content, /releases\/tag\/0\.43/);
    ...
    assert.match(readme, /releases\/tag\/0\.43/);
