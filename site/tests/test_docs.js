...
describe('shear.digital/docs', () => {
  it('is a file-tree browser with wallet, mining, vortex, vort1, reserve', () => {
    ...
    assert.match(content, /Wallet pin<\/th><td>0\.41/);
    assert.match(content, /Current pin is <strong>0\.41<\/strong>/);
    assert.match(content, /releases\/tag\/0\.41/);
    ...
    assert.match(readme, /releases\/tag\/0\.41/);
...
describe('whitepaper.shear.digital', () => {
  it('presents a Zenodo-style record with a PDF, and has no WHITEPAPER nav button', () => {
    ...
    assert.equal(pdf.includes(Buffer.from('wallet-0.41')), true);
