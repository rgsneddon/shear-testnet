...
describe('shear.digital client buttons', () => {
  it('places BIG WALLET / MINER / NODE buttons between the hero banner and the Shear box', () => {
    ...
    assert.doesNotMatch(html, /shear-wallet-0\.36-macos\.dmg/);
    assert.doesNotMatch(html, /shear-wallet-0\.31-windows\.zip/);
    assert.match(html, /shear-wallet-0\.36-android\.apk/);
    assert.match(html, /releases\/download\/0\.36\/shear-wallet-0\.36-windows\.zip/);
    assert.doesNotMatch(html, /releases\/download\/0\.35/);
    assert.match(html, /macOS — coming soon/);
    assert.match(html, /shear-wallet-0\.36-linux\.zip/);
    assert.match(html, /shear-wallet-0\.36-archlinux\.zip/);
    ...
    assert.match(html, /shear-wallet-0\.36-windows\.zip/);
    ...
    assert.match(html, /wallet <strong>0\.36<\/strong>/);
  ...
  it('WALLET nav on MAIN MEMPOOL POOL EXPLORER pins 0.36 and refuses older tags', () => {
    ...
    for (const [name, page] of Object.entries(pages)) {
      assert.match(page, /releases\/tag\/0\.36/, `${name} WALLET must pin 0.36`);
      assert.doesNotMatch(page, /releases\/tag\/0\.35/, `${name} must not pin 0.35 as current`);
      assert.doesNotMatch(page, /releases\/download\/0\.35/, `${name} must not download from tag 0.35`);
  ...
  it('admin how-to is Shear-only: wallet, mine, Reserve; no claim copy', () => {
    ...
    assert.match(admin, /shear-wallet-0\.36-/);
    assert.match(admin, /shear-wallet-0\.36-windows\.zip/);
    assert.match(admin, /shear-wallet-0\.36-android\.apk/);
