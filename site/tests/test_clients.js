...
describe('shear.digital client buttons', () => {
  it('places BIG WALLET / MINER / NODE buttons between the hero banner and the Shear box', () => {
    ...
    assert.match(html, /A launch date is not decided/);
    ...
    assert.doesNotMatch(html, /2026-09-18T21:00:00\+01:00/);
    ...
    assert.match(html, /shear-wallet-0\.49-android\.apk/);
    assert.match(html, /releases\/download\/0\.49\/shear-wallet-0\.49-windows\.zip/);
    assert.doesNotMatch(html, /releases\/download\/0\.40/);
    assert.doesNotMatch(html, /releases\/download\/0\.39/);
    assert.doesNotMatch(html, /releases\/download\/0\.36/);
    assert.match(html, /macOS — coming soon/);
    assert.match(html, /shear-wallet-0\.49-linux\.zip/);
    assert.match(html, /shear-wallet-0\.49-archlinux\.zip/);
    ...
    assert.match(html, /shear-wallet-0\.49-windows\.zip/);
    ...
    assert.match(html, /wallet <strong>0\.49<\/strong>/);
    ...
    assert.match(html, /miner <strong>2\.5<\/strong>/);
  ...
  it('WALLET nav on MAIN MEMPOOL POOL EXPLORER pins 0.49 and refuses older tags', () => {
    ...
    for (const [name, page] of Object.entries(pages)) {
      assert.match(page, /releases\/tag\/0\.49/, `${name} WALLET must pin 0.49`);
  ...
  it('puts emissions, governance, vortex columns and a full-width wallet box', () => {
    ...
    assert.match(html, /npm run solo/);
    assert.match(html, /dart run bin\/shear\.dart dest/);
    const soloRun = html.match(/id="solo-run">([\s\S]*?)<\/pre>/);
    assert.ok(soloRun, 'solo-run copy block');
    assert.doesNotMatch(soloRun[1], /npm run pool/);
    const soloStratum = html.match(/id="solo-stratum">([\s\S]*?)<\/pre>/);
    assert.ok(soloStratum, 'solo-stratum copy block');
    assert.doesNotMatch(soloStratum[1], /npm run pool/);
    assert.match(soloStratum[1], /npm run solo/);
    const soloSync = html.match(/id="solo-sync-fix">([\s\S]*?)<\/pre>/);
    assert.ok(soloSync, 'solo-sync-fix copy block');
    assert.doesNotMatch(soloSync[1], /^\s*npm run pool\s*$/m);
    assert.match(soloSync[1], /npm run solo/);
    assert.match(soloSync[1], /SHEAR_GETBLOCK_BATCH=1/);
    ...
    assert.match(html, /Debian \/ Ubuntu/);
    assert.match(html, /Fedora \/ RHEL/);
    assert.match(html, />Arch</);
    assert.match(html, /openSUSE/);
    assert.match(html, />macOS</);
    assert.match(html, />Windows</);
    assert.match(html, /dnf groupinstall -y "Development Tools"/);
    assert.match(html, /openssl-devel/);
    assert.match(html, /rpm\.nodesource\.com\/setup_20\.x/);
    assert.match(html, /pacman -Syu --needed base-devel/);
    assert.match(html, /nodejs npm rust/);
    assert.match(html, /zypper install -t pattern devel_C_C\+\+/);
    ...
    assert.match(html, /data-copy="solo-deps-windows"/);
    assert.match(html, /id="solo-os-tabs"/);
    assert.match(html, /#solo-mine=/);
    ...
    assert.match(html, /Continuum GUI <strong>0\.49<\/strong>/);
    ...
    assert.match(html, /Headers check/);
    assert.match(html, /node_prefix/);
    assert.match(html, /data-copy="solo-headers-debian"/);
    assert.match(html, /data-copy="solo-headers-fedora"/);
    assert.match(html, /data-copy="solo-headers-arch"/);
    assert.match(html, /data-copy="solo-headers-suse"/);
    assert.match(html, /data-copy="solo-headers-macos"/);
    assert.match(html, /data-copy="solo-headers-windows"/);
    assert.match(html, /data-copy="solo-nodeinc-debian"/);
    assert.match(html, /data-copy="solo-nodeinc-fedora"/);
    assert.match(html, /data-copy="solo-nodeinc-arch"/);
    assert.match(html, /data-copy="solo-nodeinc-suse"/);
    assert.match(html, /data-copy="solo-nodeinc-macos"/);
    assert.match(html, /data-copy="solo-nodeinc-windows"/);
  ...
  it('admin how-to is Shear-only: wallet, mine, Reserve; no claim copy', () => {
    ...
    assert.match(admin, /shear-wallet-0\.49-/);
    assert.match(admin, /shear-wallet-0\.49-windows\.zip/);
    assert.match(admin, /shear-wallet-0\.49-android\.apk/);
