...
(function (P) {
  P.overview = {
    ...
    html:
      ...
      '<tr><th>Wallet pin</th><td>0.41</td></tr>' +
  ...
  P.whitepaper = {
    ...
    html:
      '<p>Version 2.0 of the project note: ADMITv2, node-sync wallets, prune-1000, bootstrap at height 1000 then every 400, ShearHash-v3, emissions, Flow levy, Vortex / vort1, The Reserve. Wallet pin 0.41. Live magic <code>shear-testnet-v4</code>.</p>' +
  ...
  P.wallet = {
    ...
    html:
      '<p>The Shear wallet is a six-tab app. It does not mine. Current pin is <strong>0.41</strong> (Windows, Linux, Arch, Android; macOS coming soon). Sync is a local node at <code>127.0.0.1:18332</code> — not flyclient, not pool HTTP as the only path. Magic <code>shear-testnet-v4</code>.</p>' +
  ...
  P.install = {
    ...
    html:
      ...
      '<p>Tag: <a href="https://github.com/rgsneddon/shear-testnet/releases/tag/0.41">shear-testnet 0.41</a>. 0.39 is the previous pin; 0.37/0.36 are older — do not recut them. macOS .dmg for 0.41 is a MacBook cut (coming soon).</p>'
  };
  ...
  P.android = {
    ...
    html:
      '<p>Install <code>shear-wallet-0.41-android.apk</code> from the official 0.41 tag. Package id is <code>com.shear.shear_wallet</code>. The APK is a single fat package (all ABIs), INTERNET granted on the released file, not a split APK. Packed on Windows (Flutter + Android SDK); macOS <code>.dmg</code> stays a MacBook cut.</p>' +
      '<p>If a phone still says <strong>App not installed</strong>: uninstall the old debug-signed build first, then sideload 0.41. Do not install a Darwin zip or an unsigned copy. Camera and biometrics need the permissions already in the manifest.</p>'
  };
  ...
  P.node = {
    ...
    crumb: 'network / node',
    html:
      '<p>A node is the book. It appends, verifies, speaks P2P, and runs the GATE that lets native Flow and pinned Reserve bytecode land in the same block. Wallet 0.41 reads this node, not flyclient.</p>' +
