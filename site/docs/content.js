...
(function (P) {
  ...
  P.wallet = {
    ...
    html:
      '<p>The Shear wallet is a six-tab app. It does not mine. Current pin is <strong>0.33</strong> (macOS, Android, Linux, Arch). Sync is a local node at <code>127.0.0.1:18332</code> — not flyclient, not pool HTTP as the only path.</p>' +
  ...
  P.install = {
    ...
    html:
      ...
      '<p>Tag: <a href="https://github.com/rgsneddon/shear-testnet/releases/tag/0.33">shear-testnet 0.33</a>. 0.32 is the previous pin; 0.31 is the last flyclient pin — do not recut it.</p>'
  };
  ...
  P.android = {
    ...
    html:
      '<p>Install <code>shear-wallet-0.33-android.apk</code> from the official 0.33 tag only. Package id is <code>com.shear.shear_wallet</code>. Version code is 49 (greater than 0.32’s 48) so Android will install over a stale 0.32. The APK is a single fat package (all ABIs), INTERNET granted on the released file, not a split APK.</p>' +
      '<p>If a phone still says <strong>App not installed</strong>: uninstall the old 0.31 debug-signed build first, then sideload 0.33. Do not install a Darwin zip or an unsigned copy. Camera and biometrics need the permissions already in the manifest.</p>'
  };
  ...
  P['node-sync'] = {
    ...
    html:
      '<p>0.33 talks to a stock local node. Default seed is <code>http://127.0.0.1:18332</code>. Headers, compact blocks, and the tree root / jroot come from that node. Flyclient height sampling is not the send, balance, or history path.</p>' +
  ...
  P.node = {
    ...
    crumb: 'network / node',
    html:
      '<p>A node is the book. It appends, verifies, speaks P2P, and runs the GATE that lets native Flow and pinned Reserve bytecode land in the same block. Wallet 0.33 reads this node, not flyclient.</p>' +
