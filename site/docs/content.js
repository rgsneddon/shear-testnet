...
(function (P) {
  P.overview = {
    ...
    html:
      ...
      '<tr><th>Wallet pin</th><td>0.36</td></tr>' +
  ...
  P.whitepaper = {
    ...
    html:
      '<p>Version 2.0 of the project note: ADMITv2, node-sync wallets, prune-1000, bootstrap at height 1000 then every 400, ShearHash-v3, emissions, Flow levy, Vortex / vort1, The Reserve. Wallet pin 0.36. Live magic <code>shear-testnet-v4</code>.</p>' +
  ...
  P.wallet = {
    ...
    html:
      '<p>The Shear wallet is a six-tab app. It does not mine. Current pin is <strong>0.36</strong> (Windows, Linux, Arch, Android; macOS coming soon). Sync is a local node at <code>127.0.0.1:18332</code> — not flyclient, not pool HTTP as the only path. Magic <code>shear-testnet-v4</code>.</p>' +
  ...
  P.install = {
    ...
    html:
      ...
      '<p>Tag: <a href="https://github.com/rgsneddon/shear-testnet/releases/tag/0.36">shear-testnet 0.36</a>. 0.35 is the previous pin; 0.34 is the previous v4 pin; 0.33 is the previous v3 pin; 0.31 is the last flyclient pin — do not recut it. macOS .dmg for 0.36 is a MacBook cut (coming soon).</p>'
  };
  ...
  P.android = {
    ...
    html:
      '<p>Install <code>shear-wallet-0.36-android.apk</code> from the official 0.36 tag. Package id is <code>com.shear.shear_wallet</code>. The APK is a single fat package (all ABIs), INTERNET granted on the released file, not a split APK. Packed on Windows (Flutter + Android SDK); macOS <code>.dmg</code> stays a MacBook cut.</p>' +
      '<p>If a phone still says <strong>App not installed</strong>: uninstall the old debug-signed build first, then sideload 0.36. Do not install a Darwin zip or an unsigned copy. Camera and biometrics need the permissions already in the manifest.</p>'
  };
  ...
  P['node-sync'] = {
    ...
    html:
      '<p>0.36 talks to a stock local node on <code>shear-testnet-v4</code>. Default seed is <code>http://127.0.0.1:18332</code>. Headers, compact blocks, and the tree root / jroot come from that node. Flyclient height sampling is not the send, balance, or history path.</p>' +
  ...
  P.solo = {
    ...
    crumb: 'mining / solo',
    html:
      '<p>Solo means your node finds the block: you keep the whole 1 SHE pot plus your hash bonus. The pool process must light-verify ShearHash-v3 on that same box. Without <code>crypto/native/shearhash.node</code> (or a 2.4 <code>ShearK-Miner</code> on <code>PATH</code> / <code>SHEARK_MINER</code>), every share comes back <code>native_missing</code>.</p>' +
      ...
      'npm run pool</pre>' +
      '<p>Then point ShearK at localhost. Login is Copy dest as <code>ssa1.solo</code>.</p>' +
      '<pre>./ShearK-Miner --selftest\n' +
  ...
  P.node = {
    ...
    crumb: 'network / node',
    html:
      '<p>A node is the book. It appends, verifies, speaks P2P, and runs the GATE that lets native Flow and pinned Reserve bytecode land in the same block. Wallet 0.36 reads this node, not flyclient.</p>' +
