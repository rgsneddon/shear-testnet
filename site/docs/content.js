...
(function (P) {
  P.overview = {
    ...
    html:
      ...
      '<tr><th>Wallet pin</th><td>0.44</td></tr>' +
  ...
  P.testnet = {
    ...
    html:
      ...
      '<p>This public testnet is the privacy-class book (<code>shear-testnet-v4</code>). Mainnet <code>shear-v1</code> is not live. A launch date is not decided. Do not cut a mainnet genesis zip from these pages.</p>' +
  ...
  P.whitepaper = {
    ...
    html:
      '<p>Version 2.0 of the project note: ADMITv2, node-sync wallets, prune-1000, bootstrap at height 1000 then every 400, ShearHash-v3, emissions, Flow levy, Vortex / vort1, The Reserve. Wallet pin 0.44. Live magic <code>shear-testnet-v4</code>.</p>' +
  ...
  P.wallet = {
    ...
    html:
      '<p>The Shear wallet is a six-tab app. It does not mine. Current pin is <strong>0.44</strong> (Windows, Linux, Arch, Android; macOS coming soon). Sync is a local node at <code>127.0.0.1:18332</code> — not flyclient, not pool HTTP as the only path. Magic <code>shear-testnet-v4</code>.</p>' +
  ...
  P.install = {
    ...
    html:
      ...
      '<p>Tag: <a href="https://github.com/rgsneddon/shear-testnet/releases/tag/0.44">shear-testnet 0.44</a>. 0.43 is the previous pin; 0.42/0.40 are older — do not recut them. macOS .dmg for 0.44 is a MacBook cut (coming soon).</p>'
  };
  ...
  P.android = {
    ...
    html:
      '<p>Install <code>shear-wallet-0.44-android.apk</code> from the official 0.44 tag. Package id is <code>com.shear.shear_wallet</code>. The APK is a single fat package (all ABIs), INTERNET granted on the released file, not a split APK. Packed on Windows (Flutter + Android SDK); macOS <code>.dmg</code> stays a MacBook cut.</p>' +
      '<p>If a phone still says <strong>App not installed</strong>: uninstall the old debug-signed build first, then sideload 0.44. Do not install a Darwin zip or an unsigned copy. Camera and biometrics need the permissions already in the manifest.</p>'
  };
  ...
  P.solo = {
    ...
    html:
      ...
      'npm run solo</pre>' +
      '<p>Then point ShearK at localhost. Login is Copy dest as <code>ssa1.solo</code> (the Continuum mailbox, not a rotated dest). Do not <code>npm run pool</code> for solo. Public stratum is an optional alternate (cleartext TCP unless TLS is in front).</p>' +
      '<pre>./ShearK-Miner --selftest\n' +
      './ShearK-Miner --pool 127.0.0.1:1111 --user YOUR_SSA1.solo --threads 8</pre>'
  };
  ...
  P.reserve = {
    ...
    html:
      '<p>The Reserve is the first vortice and the only programme allowed to mint extra SHE. Open it from the Vortex tab. You lock SHE into your own portal. π SHE (about 3.14159265358 SHE) unlocks one vote for the current epoch. <strong>Your deposits</strong> shows two rows; extra deposits scroll inside that pane. Reserve Send can raise <strong>Privacy hop</strong> first so the public node does not see your device IP.</p>' +
      '<p>Privacy hop is optional. One tap pays 0.05 SHE to the pool fee dest, then connects SHEAR-HOP. Or Send without privacy hop after confirming <strong>I already use a VPN / I accept exposing my IP</strong>.</p>'
  };
  ...
  P.node = {
    ...
    crumb: 'network / node',
    html:
      '<p>A node is the book. It appends, verifies, speaks P2P, and runs the GATE that lets native Flow and pinned Reserve bytecode land in the same block. Wallet 0.44 reads this node, not flyclient.</p>' +
