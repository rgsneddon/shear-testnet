...
describe('website W-IMPL binds', () => {
  ...
  it('wallet pin 0.48 stays on explorer, pool, mempool, and the whitepaper PDF source', () => {
    const paper = read('../../site/whitepaper/index.html');
    const pdf = read('../../site/whitepaper/build_pdf.py');
    assert.match(explorer, /releases\/tag\/0\.48/);
    assert.match(pool, /releases\/tag\/0\.48/);
    assert.match(mempool, /releases\/tag\/0\.48/);
    assert.match(pdf, /pin 0\.48/);
    assert.match(pdf, /Wallet pin at publication: 0\.48/);
    assert.match(pdf, /wallet-0\.48/);
