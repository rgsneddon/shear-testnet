...
describe('explorer dests', () => {
  it('lists dest amounts and memo boolean only, no ciphertext, no rest-frame', () => {
    ...
    assert.ok(pub.json.txs.every((t) => t.from === 'coinbase' || t.from == null || t.from === ''));
    assert.ok(pub.json.txs.every((t) => t.asset === 'SHE'));
    assert.ok(pub.json.txs.every((t) => t.amountHidden === true));
    assert.ok(pub.json.txs.every((t) => t.amount == null && t.nanos == null));
    assert.ok(pub.json.txs.every((t) => t.memoCt == null && t.memoPlain == null));
    assert.ok(pub.json.txs.every((t) => t.id && t.height != null));
  ...
  it('search by height, tx id, and from–to range; circulation sums dest balances', () => {
    ...
    assert.ok(byHeight.json.txs.every((t) => t.from === 'coinbase' || !t.from));
    assert.ok(byHeight.json.txs.every((t) => t.amountHidden === true));
    assert.ok(byHeight.json.txs.every((t) => t.amount == null && t.nanos == null));
    ...
    assert.equal(JSON.stringify(circ.json).includes('ssa1'), false);
    const viaFn = explorerCirculation(store);
    assert.equal(viaFn.noteCount, circ.json.noteCount);
    assert.equal(viaFn.circulatingNanos, circ.json.circulatingNanos);
    ...
    assert.ok(recent.every((t) => t.amountHidden === true));
    assert.ok(recent.every((t) => t.amount == null && t.nanos == null));
  ...
  it('explorer page lists confirmed blocks like the pool last-30 table', () => {
    ...
    assert.match(page, /BLOCK LIST - FULL/);
    assert.doesNotMatch(page, /Last 30 blocks/);
