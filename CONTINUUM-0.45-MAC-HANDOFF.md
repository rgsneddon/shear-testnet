# Continuum 0.45 + ShearK 2.5 — MacBook only (Apple parts)
...
**Wallet tag:** `0.45` — https://github.com/rgsneddon/shear-testnet/releases/tag/0.45  
**Miner repo:** https://github.com/rgsneddon/ShearK  
**Miner tag:** `2.5` — https://github.com/rgsneddon/ShearK/releases/tag/2.5  
**This file:** https://github.com/rgsneddon/shear-testnet/blob/main/CONTINUUM-0.45-MAC-HANDOFF.md
...
This cut fixes the Android Privacy hop ANR: the 0.05 SHE fee crypto runs off the UI isolate, and the hop handshake is 15s / 3 tries. Reserve lock Sign posts a sealed public-nanos vout (`kind=lock`) so the pool spend sig verifies instead of returning `unsigned`. Fee stays 0.05 SHE. Do not recut 0.44. Do not recut ShearK.

---

## A) Continuum 0.45 macOS GUI (notarized `.dmg`)
...
# writes wallet/dist/shear-wallet-0.45-macos.dmg
gh release upload 0.45 dist/shear-wallet-0.45-macos.dmg --repo rgsneddon/shear-testnet --clobber
```
...
URL: `https://github.com/rgsneddon/shear-testnet/releases/download/0.45/shear-wallet-0.45-macos.dmg`

Then on `main`, replace any remaining site span `macOS — coming soon` with that href.

## B) Continuum 0.45 macOS CLI

Windows cannot emit a Darwin Mach-O.

```
cd ~/shear-testnet/wallet
dart compile exe bin/shear.dart -o dist/shear-0.45-macos
chmod +x dist/shear-0.45-macos
./dist/shear-0.45-macos --version    # 0.45
gh release upload 0.45 dist/shear-0.45-macos --repo rgsneddon/shear-testnet --clobber
```
...
The Windows box cannot produce an IPA. If this 0.45 cut includes iOS:

```
cd ~/shear-testnet/wallet
flutter build ipa --release --build-name=0.45.0 --build-number=62
...
Wallet public pin **0.45** (`kWalletVersion` / `kCliVersion`), store `0.45.0+62`. Miner pin **ShearK 2.5**.
