...
linux_out="$DIST/shear-wallet-0.11-linux.zip"
arch_out="$DIST/shear-wallet-0.11-archlinux.zip"
...
linux_out = os.path.join(dist, "shear-wallet-0.11-linux.zip")
with zipfile.ZipFile(linux_out, "w", zipfile.ZIP_DEFLATED) as z:
    add_tree(z, bundle)
print("wrote", linux_out, os.path.getsize(linux_out))

arch_out = os.path.join(dist, "shear-wallet-0.11-archlinux.zip")
...
for name in (linux_out, arch_out):
    ...
    if "archlinux" in name:
        ...
        if "pkgver=0.11" not in pkg or "pkgver=0.11.0" in pkg:
            sys.exit("arch PKGBUILD not two-part 0.11")
print("ok")
PY
