...
linux_out="$DIST/shear-wallet-0.0.8-linux.zip"
arch_out="$DIST/shear-wallet-0.0.8-archlinux.zip"
...
linux_out = os.path.join(dist, "shear-wallet-0.0.8-linux.zip")
with zipfile.ZipFile(linux_out, "w", zipfile.ZIP_DEFLATED) as z:
    add_tree(z, bundle)
print("wrote", linux_out, os.path.getsize(linux_out))

arch_out = os.path.join(dist, "shear-wallet-0.0.8-archlinux.zip")
...
for name in (linux_out, arch_out):
    ...
    if "archlinux" in name:
        ...
        if "pkgver=0.0.8" not in pkg:
            sys.exit("arch PKGBUILD not 0.0.8")
print("ok")
PY
