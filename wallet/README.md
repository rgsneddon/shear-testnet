...
Releases: https://github.com/rgsneddon/shear-testnet/releases/tag/0.43

| Platform | Install |
|----------|---------|
| Windows | Unzip `shear-wallet-0.43-windows.zip`, run `shear_wallet.exe` |
| Linux | Unzip `shear-wallet-0.43-linux.zip`, run `./shear_wallet` |
| Arch | Unzip `shear-wallet-0.43-archlinux.zip` (includes `PKGBUILD`) or run `./shear_wallet` |
| Android | Sideload `shear-wallet-0.43-android.apk` (`com.shear.shear_wallet`). Uninstall any old debug-signed build first |
| macOS | `.dmg` is packed on the MacBook only — see `CONTINUUM-0.43-MAC-HANDOFF.md` until the asset 200s |
...
Idle 0.43 polls the tip every ~4s when nothing moved; 1s only when the tip, a pending receive, or history is behind.
