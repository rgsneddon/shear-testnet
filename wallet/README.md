...
Releases: https://github.com/rgsneddon/shear-testnet/releases/tag/0.40

| Platform | Install |
|----------|---------|
| Windows | Unzip `shear-wallet-0.40-windows.zip`, run `shear_wallet.exe` |
| Linux | Unzip `shear-wallet-0.40-linux.zip`, run `./shear_wallet` |
| Arch | Unzip `shear-wallet-0.40-archlinux.zip` (includes `PKGBUILD`) or run `./shear_wallet` |
| Android | Sideload `shear-wallet-0.40-android.apk` (`com.shear.shear_wallet`). Uninstall any old debug-signed build first |
...
Idle 0.40 polls the tip every ~4s when nothing moved; 1s only when the tip, a pending receive, or history is behind.
