...
Custody hasher Spendable is the last successful pool reconstruct (sealed hash notes plus real pool-withdraw landings). A fatter note `amount`, a cached blockfound pot, a sibling dest the sweep did not write, or a 504 does not put pot-class SHE back onto that dest. Force-sync is done only when every owned dest's live balance wrote. The pane repaints from that owned sum, so a sibling dest correction shows even when the mailbox figure stays put. Owed-π stays display-only. Admin custody display is not Continuum Spendable. Sign on Reserve withdraw does not credit Continuum or clear In Reserve until the pool accepts the withdraw tx. An archive without a pool book debits confirmed locks and does not show that principal again as Spendable. Public tag **0.49** (store `0.49.0+71`). Download: https://github.com/rgsneddon/shear-testnet/releases/tag/0.49 — see `CONTINUUM-0.49-HANDOFF.md`. ShearK pin stays **2.5**. Tag 0.48 packs were not replaced. macOS `.dmg` stays the MacBook handoff (`CONTINUUM-0.46-MAC-HANDOFF.md`).
