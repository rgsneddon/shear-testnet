...
/// Inspects the packed 0.32 fat APK when present.
File _apk() {
  for (final p in [
    '../dist/shear-wallet-0.32-android.apk',
    'dist/shear-wallet-0.32-android.apk',
    '${Directory.current.path}/../dist/shear-wallet-0.32-android.apk',
  ]) {
    final f = File(p);
    if (f.existsSync()) return f;
  }
  return File('../dist/shear-wallet-0.32-android.apk');
}
...
void main() {
  test('0.32 APK is a fat installable package (applicationId, versionCode > 47, INTERNET)', () {
    ...
    expect(RegExp(r"versionName='0\.32(\.0)?'").hasMatch(out), isTrue);
    final code = RegExp(r"versionCode='(\d+)'").firstMatch(out);
    expect(code, isNotNull);
    expect(int.parse(code!.group(1)!), greaterThan(47));
