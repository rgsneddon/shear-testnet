...
/// Inspects the **built** `shear-wallet-0.8-windows.zip` (not a mocked listing).
/// Pack with `python wallet/pack/zip_windows.py` after `flutter build windows --release`.
File _shippedWindowsZip() {
  final candidates = <File>[
    File('../dist/shear-wallet-0.8-windows.zip'),
    File('dist/shear-wallet-0.8-windows.zip'),
    File('${Directory.current.path}/../dist/shear-wallet-0.8-windows.zip'),
  ];
...
void main() {
  test('kWalletVersion public pin is two-part 0.8 (not 0.8.0)', () {
  ...
  test('built shear-wallet-0.8-windows.zip is a Flutter runner with no miner', () {
  ...
  test('built shear-wallet-0.8-linux.zip has shear_wallet and no miner', () {
    final zip = _zipAt('shear-wallet-0.8-linux.zip');
  ...
  test('built shear-wallet-0.8-archlinux.zip has PKGBUILD pkgver=0.8 and no miner', () {
    final zip = _zipAt('shear-wallet-0.8-archlinux.zip');
    if (!zip.existsSync()) return;
    expect(zip.lengthSync(), greaterThan(1 * 1024 * 1024));
    final names = _zipNames(zip);
