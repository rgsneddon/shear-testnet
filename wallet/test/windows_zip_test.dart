...
/// Inspects the **built** `shear-wallet-0.21-windows.zip` (not a mocked listing).
/// Pack with `python wallet/pack/zip_windows.py` after `flutter build windows --release`.
File _shippedWindowsZip() {
  final candidates = <File>[
    File('../dist/shear-wallet-0.21-windows.zip'),
    File('dist/shear-wallet-0.21-windows.zip'),
    File('${Directory.current.path}/../dist/shear-wallet-0.21-windows.zip'),
  ];
...
void main() {
  test('kWalletVersion public pin is two-part 0.21 (not 0.14.0)', () {
    ...
    expect(linux, contains('--build-name=0.21'));
    expect(linux, isNot(contains('--build-name=0.14')));
    final macos = File('pack_macos.sh').readAsStringSync();
    expect(macos, contains('VER=0.21'));
    expect(macos, isNot(contains('VER=0.14')));
  });

  test('built shear-wallet-0.21-windows.zip is a Flutter runner with no miner', () {
  ...
  test('built shear-wallet-0.21-linux.zip has shear_wallet and no miner', () {
    final zip = _zipAt('shear-wallet-0.21-linux.zip');
  ...
  test('built shear-wallet-0.21-archlinux.zip has PKGBUILD pkgver=0.21 and no miner', () {
    final zip = _zipAt('shear-wallet-0.21-archlinux.zip');
    if (!zip.existsSync()) return;
    expect(zip.lengthSync(), greaterThan(1 * 1024 * 1024));
    final names = _zipNames(zip);
    ...
    expect(listed.exitCode, 0, reason: listed.stderr.toString());
    expect(listed.stdout.toString(), contains('pkgver=0.21'));
    expect(listed.stdout.toString().contains('pkgver=0.21.0'), isFalse);
    for (final n in names) {
      final base = n.split('/').last;
      expect(base.toLowerCase(), isNot(equals('shear-miner')));
      expect(base, isNot(equals('ShearK-Miner')));
