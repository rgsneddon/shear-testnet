...
void main() {
  test('release AndroidManifest grants INTERNET; debug/profile overlays are not the shipped grant', () {
    ...
    expect(main.readAsStringSync().contains('android:label="Shear 0.54.0"'), isTrue);
    ...
    expect(winMain.contains('L"Shear 0.54.0"'), isTrue);
    expect(winMain.contains('Shear 0.6'), isFalse);
    expect(winRc.contains('"Shear 0.54.0"'), isTrue);
    expect(winRc.contains('Shear 0.7'), isFalse);
    expect(linuxApp.contains('"Shear 0.54.0"'), isTrue);
  ...
  test('read-sync source defaults to 127.0.0.1 before the public seed', () {
    ...
    expect(File('pubspec.yaml').readAsStringSync(), contains('version: 0.54.0+76'));
    expect(File('lib/shear_cli.dart').readAsStringSync(), contains("const kCliVersion = '0.54.0'"));
  });
  ...
  test('kWalletVersion == 0.54.0 and 400-day APR uses observed average bps', () {
    expect(kWalletVersion, '0.54.0');
