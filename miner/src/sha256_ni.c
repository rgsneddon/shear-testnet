/*
 * SHA-NI (Intel SHA extensions) multi-block SHA-256.
 * Compile this file only with -msse4.1 -msha. Never call without CPUID SHA.
 * Hashes the full padded message (every 64-byte block plus SHA-256 padding).
 *
 * Transform follows Intel's SHA-NI programming note / public-domain
 * SHA-Intrinsics (Jeffrey Walton).
 */
#include "sha256.h"
...
void sha256_oneshot_ni(const uint8_t *data, size_t len, uint8_t out[32]) {
  ...
  size_t i = 0;
  for (; i + 64 <= len; i += 64) sha256_compress_ni(state, data + i);
  uint8_t tail[128];
  ...
  sha256_compress_ni(state, tail);
  if (pad_blocks == 2) sha256_compress_ni(state, tail + 64);
...
void sha256_oneshot_x8_ni(const uint8_t *const data[8], size_t len, uint8_t out[8][32]) {
  for (int lane = 0; lane < 8; lane++) sha256_oneshot_ni(data[lane], len, out[lane]);
}
